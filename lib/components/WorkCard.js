...
export default function WorkCard({ work, idx }) {
    ...
    return (
        <AnimatedSection variants={variants}>
            <div className={`project shadow-lg border-2 md:border-0 border-stone-300 md:shadow-none w-full ${idx % 2 === 0 ? 'left' : 'right'}`}>
                ...
                <div className="project-content text-left">
                    <h3 className="font-semibold uppercase tracking-widest text-sm text-stone-400">{work.data.featured ? 'Featured Project' : 'Project'}</h3>
                    ...
                    <div className="flex links space-x-4 w-full mt-4">
                        <ExternalLinkIcon className="h-6 text-stone-800 hover:text-stone-500 cursor-pointer" />
                        <FaGithub className="h-6 text-stone-800 hover:text-stone-500 cursor-pointer" />
                    </div>
                </div>
            </div>
        </AnimatedSection>
